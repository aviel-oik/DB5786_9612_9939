...
### שאילתה 2: מטוסים שביצעו טיסות ארוכות מהממוצע הכללי


**הסיפור:**  
מחלקת התחזוקה רוצה לאתר את כל המטוסים שטסו טיסות שמשך הזמן שלהן ארוך יותר מהממוצע של כלל הטיסות במערכת, כדי לתזמן להם בדיקות בטיחות מחמירות יותר.


**הסבר על ההבדל והיעילות:**  
הבדל: גרסה א' מחשבת את הממוצע בתוך תת-שאילתה ב-WHERE. גרסה ב' משתמשת בביטוי WITH (CTE) כדי לחשב את הממוצע פעם אחת מראש.
מה יותר יעיל? גרסה ב' (CTE). בגרסה א', בסיס הנתונים עלול (תלוי ב-Optimizer) לחשב את הממוצע מחדש עבור כל שורה (Correlated Subquery), בעוד שב-CTE הממוצע מחושב פעם אחת בלבד בתחילת הריצה ונשמר כערך קבוע, מה שחוסך זמן עיבוד יקר.
